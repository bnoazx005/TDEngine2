...
TDE2_TEST_FIXTURE("Physics2D Tests")
{
	TDE2_TEST_CASE("TestPhysics2DSystem_CreateKinematicAndDynamicPhysicsBodies_FirstOneShouldStayAtSamePlaceAnotherOneGoDownDueToGravityForce")
	{
		...
		pTestCase->ExecuteAction([&]
			{
				...
				pKinematicEntity->AddComponent<CBoxCollisionObject2D>();
				
				CPhysicsBody2D* pKinematicObject = pKinematicEntity->AddComponent<CPhysicsBody2D>();
				...
				pDynamicEntity->AddComponent<CBoxCollisionObject2D>();

				CPhysicsBody2D* pDynamicObject = pDynamicEntity->AddComponent<CPhysicsBody2D>();
