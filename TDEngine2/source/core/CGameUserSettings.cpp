...
namespace TDEngine2
{
	...
	E_RESULT_CODE CGameUserSettings::Init(IConfigFileReader* pConfigFileReader)
	{
		...
		mCurrent.mShadowCascadesSplits.x = static_cast<F32>(pConfigFileReader->GetFloat("graphics", "shadow_cascades_splits.0", 0.020f));
		mCurrent.mShadowCascadesSplits.y = static_cast<F32>(pConfigFileReader->GetFloat("graphics", "shadow_cascades_splits.1", 0.045f));
		mCurrent.mShadowCascadesSplits.z = static_cast<F32>(pConfigFileReader->GetFloat("graphics", "shadow_cascades_splits.2", 0.08f));
