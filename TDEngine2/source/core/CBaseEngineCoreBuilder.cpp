...
namespace TDEngine2
{
	...
	E_RESULT_CODE CBaseEngineCoreBuilder::_configureEditorsManager()
	{
		...
		IEditorWindow* pLevelEditorWindow = CreateLevelEditorWindow(
			{ 
				pEditorsManager.Get(),
				pInputContext.Get(), 
				mpWindowSystemInstance.Get(), 
				pDebugUtility, 
				pSceneManager.Get(), 
	...
	E_RESULT_CODE CBaseEngineCoreBuilder::_configureSceneManager()
	{
		...
		auto pWorld = TPtr<IWorld>(CreateWorld(mpWindowSystemInstance->GetEventManager(), mpJobManagerInstance, result));
