...
namespace TDEngine2
{
	...
	E_RESULT_CODE CWorld::Init(TPtr<IEventManager> pEventManager, TPtr<IJobManager> pJobManager)
	{
		...
		mpEventManager->EnableBufferingForEventType<TOnHierarchyChangedEvent>();
		mpEventManager->EnableBufferingForEventType<TOnNewWorldInstanceCreated>();
		mpEventManager->EnableBufferingForEventType<TOnNewComponentFactoryRegistered>();
