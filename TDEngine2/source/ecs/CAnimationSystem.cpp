...
namespace TDEngine2
{
	...
	struct TAnimationUpdateSystemAutoInitializer : TSystemAutoInitializer
	{
		virtual ~TAnimationUpdateSystemAutoInitializer() = default;
		...
		ISystem* GetSystem(IWorld* pWorld, const TRequestSubsystemCallback& subsystemsProviderCallback)
		{
			...
			pSystemInstance = CreateAnimationSystem(
				PolymorphicCast<IResourceManager*>(subsystemsProviderCallback(E_ENGINE_SUBSYSTEM_TYPE::EST_RESOURCE_MANAGER)),
				PolymorphicCast<IEventManager*>(subsystemsProviderCallback(E_ENGINE_SUBSYSTEM_TYPE::EST_EVENT_MANAGER)), result);
	...
	TDE2_REGISTER_SYSTEM(TAnimationUpdateSystemAutoInitializer);
}