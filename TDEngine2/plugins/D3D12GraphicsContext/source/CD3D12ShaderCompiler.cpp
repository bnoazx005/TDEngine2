...
namespace TDEngine2
{
	...
	/*!
		...
		\brief The class represents main compiler of shaders for D3D12 GAPI
	*/
	...
	class CD3D12ShaderCompiler : public CBaseShaderCompiler
	{
		...
		protected:
			DECLARE_INTERFACE_IMPL_PROTECTED_MEMBERS(CD3D12ShaderCompiler)
	...
	{
		TDE2_PROFILER_SCOPE("CD3D12ShaderCompiler::Compile");
	...
	TResult<std::vector<U8>> CD3D12ShaderCompiler::_compileShaderStage(E_SHADER_STAGE_TYPE shaderStage, const std::string& source,
																	   TShaderMetadata& shaderMetadata) const
	{
		TDE2_PROFILER_SCOPE("CD3D12ShaderCompiler::_compileShaderStage");
		...
		{
			LOG_ERROR(Wrench::StringUtils::Format("[CD3D12ShaderCompiler] Compilation report:\n {0}", pErrors->GetStringPointer()));
		}
		...
		if (FAILED(hrResult))
		{
			LOG_ERROR(Wrench::StringUtils::Format("[CD3D12ShaderCompiler] Compilation failed: {0}", shaderMetadata.mShaderName));
			return Wrench::TErrValue<E_RESULT_CODE>(RC_FAIL);
		}
	...
	CD3D12ShaderCompiler::TUniformBuffersMap CD3D12ShaderCompiler::_processUniformBuffersDecls(const TStructDeclsMap& structsMap, CTokenizer& tokenizer) const
	{
	...
	E_SHADER_FEATURE_LEVEL CD3D12ShaderCompiler::_getTargetVersionFromStr(const std::string& ver) const
	{
		if (ver == "3_0")
		{
			return SFL_3_0;
	...
	USIZE CD3D12ShaderCompiler::_getBuiltinTypeSize(const std::string& type, const std::function<void(const std::string&)> typeProcessor) const
	{
	...
	CD3D12ShaderCompiler::TShaderResourcesMap CD3D12ShaderCompiler::_processShaderResourcesDecls(CTokenizer& tokenizer) const
	{
	...
	E_SHADER_RESOURCE_TYPE CD3D12ShaderCompiler::_isShaderResourceType(const std::string& token) const
	{
