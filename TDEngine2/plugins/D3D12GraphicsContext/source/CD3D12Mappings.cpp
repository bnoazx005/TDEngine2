...
namespace TDEngine2
{
	/*U32 CD3D12Mappings::GetAccessFlags(E_BUFFER_USAGE_TYPE type)
	{
		U32 accessFlags = 0x0;
		...
		switch (type)
		{
			case E_BUFFER_USAGE_TYPE::DYNAMIC:
				accessFlags |= D3D12_CPU_ACCESS_WRITE;
				break;
		}
	...
	D3D12_USAGE CD3D12Mappings::GetUsageType(E_BUFFER_USAGE_TYPE type)
	{
		switch (type)
		{
			case E_BUFFER_USAGE_TYPE::DYNAMIC:
	...
	std::string CD3D12Mappings::GetShaderTargetVerStr(E_SHADER_STAGE_TYPE stageType, E_SHADER_FEATURE_LEVEL version)
	{
		std::string result;
	...
	U32 CD3D12Mappings::GetNumOfChannelsOfFormat(E_FORMAT_TYPE format)
	{
		switch (format)
		{
			case FT_BYTE1:
	...
	const C8* CD3D12Mappings::GetSemanticTypeName(E_VERTEX_ELEMENT_SEMANTIC_TYPE semanticType)
	{
		switch (semanticType)
		{
			case VEST_POSITION:
	...
	D3D12_PRIMITIVE_TOPOLOGY CD3D12Mappings::GetPrimitiveTopology(E_PRIMITIVE_TOPOLOGY_TYPE topologyType)
	{
		switch (topologyType)
		{
	...
	{		
		...
		return D3D12_FILTER_MIN_MAG_MIP_POINT; /// point filter is used by default
	}

	D3D12_TEXTURE_ADDRESS_MODE CD3D12Mappings::GetTextureAddressMode(E_ADDRESS_MODE_TYPE addressMode)
	{
		switch (addressMode)
		{
	...
	D3D12_BLEND_OP CD3D12Mappings::GetBlendOpType(E_BLEND_OP_TYPE opType)
	{
		switch (opType)
		{
	...
	/*D3D12_TEXTURECUBE_FACE CD3D12Mappings::GetCubemapFace(E_CUBEMAP_FACE faceType)
	{
		switch (faceType)
		{
			...
			case E_CUBEMAP_FACE::NEGATIVE_X:
				return D3D12_TEXTURECUBE_FACE_NEGATIVE_X;
			case E_CUBEMAP_FACE::POSITIVE_Y:
				return D3D12_TEXTURECUBE_FACE_POSITIVE_Y;
			case E_CUBEMAP_FACE::NEGATIVE_Y:
				return D3D12_TEXTURECUBE_FACE_NEGATIVE_Y;
			case E_CUBEMAP_FACE::POSITIVE_Z:
				return D3D12_TEXTURECUBE_FACE_POSITIVE_Z;
			case E_CUBEMAP_FACE::NEGATIVE_Z:
				return D3D12_TEXTURECUBE_FACE_NEGATIVE_Z;
			default:
				return D3D12_TEXTURECUBE_FACE_POSITIVE_X;
		}
	}*/
	...
	D3D12_CULL_MODE CD3D12Mappings::GetCullMode(const E_CULL_MODE& cullMode)
	{
		switch (cullMode)
		{
