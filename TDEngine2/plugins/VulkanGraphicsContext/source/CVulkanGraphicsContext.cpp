...
namespace TDEngine2
{
	...
	*/
	...
	E_RESULT_CODE CVulkanGraphicsContext::SetTexture(U32 slot, TTextureHandleId textureHandle, bool isWriteEnabled, const std::optional<U32>& subresourceId)
	{
	...
	{
		VkPipelineStageFlags2 result = HasEnumFlag(layout, E_RESOURCE_LAYOUT::SHADER_RESOURCE) ? VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT : 0x0;

		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::UAV_RESOURCE) ? VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::CONSTANT_BUFFER) ? VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::COPY_SRC) ? VK_PIPELINE_STAGE_2_TRANSFER_BIT : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::COPY_DEST) ? VK_PIPELINE_STAGE_2_TRANSFER_BIT : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::RENDER_TARGET) ? VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::DEPTH_STENCIL) ? (VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT) : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::DEPTH_STENCIL_READONLY) ? (VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT) : 0x0;
		result |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::INDIRECT_ARGS_BUFFER) ? VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT : 0x0;
	...
	{
		VkAccessFlags2 flags = HasEnumFlag(layout, E_RESOURCE_LAYOUT::SHADER_RESOURCE) ? VK_ACCESS_2_SHADER_READ_BIT : 0x0;

		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::UAV_RESOURCE) ? (VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_SHADER_WRITE_BIT) : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::CONSTANT_BUFFER) ? VK_ACCESS_2_UNIFORM_READ_BIT : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::COPY_SRC) ? VK_ACCESS_2_TRANSFER_READ_BIT : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::COPY_DEST) ? VK_ACCESS_2_TRANSFER_WRITE_BIT : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::RENDER_TARGET) ? (VK_ACCESS_2_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT) : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::DEPTH_STENCIL) ? (VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT) : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::DEPTH_STENCIL_READONLY) ? VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::DEPTH_STENCIL_READONLY) ? VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT : 0x0;
		flags |= HasEnumFlag(layout, E_RESOURCE_LAYOUT::INDIRECT_ARGS_BUFFER) ? VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT : 0x0;
