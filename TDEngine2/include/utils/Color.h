...
namespace TDEngine2
{
	...
	*/
	...
	TDE2_API inline E_RESULT_CODE Serialize(class IArchiveWriter* pWriter, const TColor32F& value)
	{
		return pWriter->SetUInt32("type_id", static_cast<U32>(GetTypeId<TColor32F>::mValue)) | SaveColor32F(pWriter, value);
	}

	TDE2_API inline E_RESULT_CODE Serialize(class IArchiveWriter* pWriter, const std::string& name, const TColor32F& value)
	{
	...
	template <>	struct TDeserializer<TColor32F> 
	{ 
		TDE2_API static inline TResult<TColor32F> Deserialize(IArchiveReader* pReader, const std::string& name)
		{
