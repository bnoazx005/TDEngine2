...
bool lock::impl::is_convertible(format f) const {
  ...
  else if (IsClipboardFormatAvailable(static_cast<UINT>(f)))
    return true;
  else
    return false;
}
...
bool lock::impl::set_data(format f, const char* buf, size_t len) {
  ...
  if (f == text_format()) {
    if (len > 0) {
      int reqsize = MultiByteToWideChar(CP_UTF8, 0, buf, static_cast<int>(len), NULL, 0);
      if (reqsize > 0) {
        ++reqsize;
    ...
    if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
      ...
      if (hglobal) {
        ...
        if (lpstr) {
          ...
          if (reqsize <= len) {
            WideCharToMultiByte(CP_UTF8, 0, lpstr, -1,
                                buf, static_cast<int>(reqsize), nullptr, nullptr);
            result = true;
          }
          GlobalUnlock(hglobal);
        }
...
bool lock::impl::get_image(image& output_img) const {
  ...
  if (png_format && IsClipboardFormatAvailable(png_format)) {
    ...
    if (png_handle) {
      ...
      bool result = win::read_png(png_data, static_cast<UINT>(png_size), &output_img, nullptr);
...
format register_format(const std::string& name) {
  int reqsize = 1+MultiByteToWideChar(CP_UTF8, 0,
                                      name.c_str(), static_cast<int>(name.size()), NULL, 0);
