...
- 2D/3D physics systems and its components were refactored. Now separate components **CPhysicsBody2D, CPhysicsBody3D** are used to specify collision type and mass of an object. Part of interfaces were removed from the code.
...
- A new container type **CFixedEventQueue<T, USIZE>** was defined.

- The core API of **IEventManager** was changed, not it supports buffering events into queues.

- A new approach to change ECS world's state was partially integrated which is **IECSCommandBuffer**.
...
- The API of **IGraphicsContext** was extended with new methods to describe resource barriers including a special type (DebugBarrier).
...
- A new graphics context implementation was integrated which provides Vulkan GAPI support.
...
- All rendering systems' work is based on programmable vertex pulling technique now.
...
- Add implementation of GPU particles compute shaders under GL graphics context.
