...
void main(void)
{
	vec3 viewDir = normalize(VertWorldPos);
    ...
    // Sample the textures.
    vec3 rayleigh = TEX2D(RayleighLUT, vec2(u, 1.0 - v)).rgb;
    vec3 mie = TEX2D(MieLUT, vec2(u, 1.0 - v)).rgb;
	...
	FragColor = GammaToLinear(vec4(rgb, 1.0));
}
